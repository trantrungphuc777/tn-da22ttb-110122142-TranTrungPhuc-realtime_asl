/**
 * Script seed lại toàn bộ 30 huy hiệu hệ thống
 * Chạy: node seedBadges.js
 */
...
async function seed() {
    try {
        ...
        // Xóa toàn bộ badge hệ thống cũ
        const deleted = await Badge.deleteMany({ isSystem: true });
        console.log(`🗑️  Deleted ${deleted.deletedCount} old system badges`);

        // Insert toàn bộ 30 badge mới
        const inserted = await Badge.insertMany(ALL_BADGES);
        console.log(`✅ Inserted ${inserted.length} new badges`);

        // Hiển thị tóm tắt theo rarity
        const summary = ALL_BADGES.reduce((acc, b) => {
            acc[b.rarity] = (acc[b.rarity] || 0) + 1;
            return acc;
        }, {});
        console.log('\n📊 Badge summary:');
        Object.entries(summary).forEach(([r, c]) => console.log(`   ${r}: ${c} badges`));
        console.log(`\n🎉 Total: ${inserted.length} badges seeded successfully!`);
