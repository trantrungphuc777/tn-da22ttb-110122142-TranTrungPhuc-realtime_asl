...
const srvUri = 'mongodb+srv://admin:Demo%40123@cluster0.yyx5afj.mongodb.net/?appName=Cluster0';
const dbName = 'signlanguage_db';

// Resolve SRV record manually
function resolveSrv() {
    return new Promise((resolve, reject) => {
        dns.resolveSrv('_mongodb._tcp.cluster0.yyx5afj.mongodb.net', (err, addresses) => {
            if (err) {
                // Fallback: try Google DNS
                dns.resolveSrv('_mongodb._tcp.cluster0.yyx5afj.mongodb.net', { servername: '8.8.8.8' }, (err2, addrs2) => {
                    if (err2) reject(err2);
                    else resolve(addrs2);
                });
            } else {
...
async function seedDatabase() {
    try {
        ...
        // Resolve SRV first
        console.log('📡 Resolving DNS SRV record...');
        let addresses;
        try {
            addresses = await resolveSrv();
        } catch (dnsError) {
            console.log('⚠️ SRV resolve failed, trying direct connection...');
            addresses = [
                { name: 'cluster0-shard-00-00.yyx5afj.mongodb.net', port: 27017 },
                { name: 'cluster0-shard-00-01.yyx5afj.mongodb.net', port: 27017 },
                { name: 'cluster0-shard-00-02.yyx5afj.mongodb.net', port: 27017 }
            ];
        }
        
        console.log('✅ DNS resolved:', addresses.map(a => `${a.name}:${a.port}`));
        
        // Build standard connection string
        const hosts = addresses.map(a => `${a.name}:${a.port}`).join(',');
        const uri = `mongodb://admin:Demo%40123@${hosts}/${dbName}?authSource=admin&ssl=true`;
