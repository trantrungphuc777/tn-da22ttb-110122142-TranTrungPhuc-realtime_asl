// Temporary seed API endpoint - thêm vào server.js hoặc chạy riêng
...
app.get('/seed', async (req, res) => {
    try {
        ...
        for (const instructor of instructors) {
            ...
            if (existing) {
                console.log(`  ⚠️ ${instructor.fullName} already exists`);
                ...
                console.log(`  ✅ Created: ${instructor.fullName}`);
            }
        }
        ...
        for (let i = 0; i < students.length; i++) {
            ...
            if (existing) {
                console.log(`  ⚠️ ${student.fullName} already exists`);
            } else {
                await User.create({
                    ...student,
                ...
                console.log(`  ✅ Created: ${student.fullName} (assigned to ${instructors[instructorIndex].fullName})`);
            }
        }

        res.json({ success: true, message: 'Seed completed! 8 Instructors + 40 Students created.' });
    } catch (error) {
...
mongoose.connect(MONGO_URI)
    .then(() => {
        ...
        app.listen(5001, () => {
            console.log('🚀 Seed server running on http://localhost:5001');
            console.log('📌 Visit http://localhost:5001/seed to seed data');
        });
    })
